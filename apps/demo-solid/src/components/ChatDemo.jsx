...
export const ChatDemo = (props) => {
  const [messages, setMessages] = createSignal([]);
  const [inputText, setInputText] = createSignal('');
  const [isProcessing, setIsProcessing] = createSignal(false);
  const [responseIndex, setResponseIndex] = createSignal(0);
  ...
  return (
    <page>
      ...
      <gridlayout rows="*, auto" class="bg-slate-100">
        {/* Messages list */}
        <scrollview row="0" ref={(el) => (scrollViewRef = el)}>
                ...
                >
                  {message.isUser ? (
                    ...
                    <stacklayout col="0" class="bg-white rounded-2xl rounded-bl-sm px-4 py-3 max-w-[85%] shadow-sm">
                      <Streamdown content={message.streamedContent} config={config} />
                      {message.isStreaming && (
        ...
        <gridlayout row="1" columns="*, auto" class="bg-white border-t-[1] border-slate-200 px-4 py-3">
          <textview
            col="0"
            text={inputText()}
            on:textChange={(e) => setInputText(e.value)}
            hint="Ask something..."
