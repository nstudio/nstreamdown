...
import { showMenu, MenuConfig } from '../utils';
...
> *Every element you see is a native platform view*
...
@Component({
  ...
  template: `
    <GridLayout rows="auto, *, auto" class="bg-slate-100 dark:bg-slate-900">
      <!-- Native ActionBar -->
      <ActionBar flat="true" class="text-black dark:text-white bg-slate-50 dark:bg-slate-900" title="AI Chat">
      ...
      <ScrollView row="1" #scrollView>
        <StackLayout class="p-4">
          @for (message of messages(); track $index) {
            ...
            @if (message.role === 'assistant') {
              <GridLayout columns="30, *" class="mb-3">
                ...
                <GridLayout col="0" rows="26" columns="26" class="bg-purple-600 dark:bg-purple-700 rounded-full mr-1 h-[26] w-[26]">
                  <Label text="AI" class="text-white text-[10] font-bold text-center"></Label>
                </GridLayout>
          ...
          <!-- Typing indicator -->
          @if (isTyping()) {
            <GridLayout columns="30, auto" class="mb-3">
              <GridLayout col="0" rows="26" columns="26" class="bg-purple-600 dark:bg-purple-700 rounded-full mr-1 h-[26] w-[26]">
                <Label text="AI" class="text-white text-[10] font-bold text-center"></Label>
              </GridLayout>
      ...
      <GridLayout row="2" columns="*, auto" class="bg-white dark:bg-slate-800 px-4 py-3 border-t border-slate-200 dark:border-slate-700">
        ...
        <StackLayout col="1" #menuAnchor class="bg-slate-100 dark:bg-slate-700 rounded-lg px-3 py-2" (tap)="showOptionsMenu($event)">
          <Label text="⋯" class="text-sm text-slate-600 dark:text-slate-300 font-medium"></Label>
        </StackLayout>
      </GridLayout>
    </GridLayout>
  `,
